...
def attack(char_name: str, char_class: str) -> str:
    """Рассчитан урон противнику.
    Реализация команды attack.
    """
    damage: int = 0
...
def defence(char_name: str, char_class: str) -> str:
    """Реализация команды defence."""
    safe: int = 0
    if char_class == 'warrior':
        safe = 10 + randint(5, 10)
        return (f'{char_name} блокировал {safe} урона')
    if char_class == 'mage':
        safe = 10 + randint(-2, 2)
        return (f'{char_name} блокировал {safe} урона')
    if char_class == 'healer':
...
def special(char_name: str, char_class: str) -> str:
    """Реализовано применение специального умения."""
    power: int = 0
    if char_class == 'warrior':
        power = 80 + 25
        return (f'{char_name} применил специальное '
                f'умение «Выносливость {power}»')
    if char_class == 'mage':
        power = 5 + 40
        return (f'{char_name} применил специальное умение «Атака {power}»')
    if char_class == 'healer':
        power = 10 + 30
        return (f'{char_name} применил специальное умение «Защита {power}»')
    return (f'{char_name} не применил специальное умение')


def start_training(char_name: str, char_class: str) -> str:
    """Прописаны команды для игры."""
    if char_class == 'warrior':
